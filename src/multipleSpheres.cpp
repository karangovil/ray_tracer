...
int main()
{
    ...
    material m_floor;
    m_floor.set_color(color {1.0f, 0.9f, 0.9f});
    ...
    material m_middle;
    m_middle.set_color(color {0.5f, 1.0f, 0.1f});
    ...
    material m_right;
    m_right.set_color(color {1.0f, 0.8f, 0.1f});
    ...
    material m_left;
    m_left.set_color(color {1.0f, 0.8f, 0.1f});
    m_left.set_diffuse(0.7);
    m_left.set_specular(0.3);
    left.set_material(m_left);
    
    world w;
    w.set_light(point_light {point(-10.0, 10.0, -10.0), color {1.0f, 1.0f, 1.0f}});

    std::vector<std::shared_ptr<object>> objs;
    objs.emplace_back(std::make_shared<sphere>(floor));
    ...
    canvas_to_ppm(canv, "multipleSpheres");
